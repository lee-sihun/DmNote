...
export const pickerRowClass = (mode: PickerRenderMode): string =>
  mode === 'page'
    ? 'w-full h-[30px] px-[8px] rounded-[6px] text-style-4 transition-colors flex items-center gap-[4px] group'
...
export const pickerMoreButtonClass = (mode: PickerRenderMode): string =>
  mode === 'page'
    ? 'w-[28px] h-[28px] rounded-[6px] transition-all flex items-center justify-center shrink-0'
