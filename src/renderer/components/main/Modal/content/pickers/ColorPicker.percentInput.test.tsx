...
describe('ColorPicker 불투명도 % 입력', () => {
  ...
  describe('hex 입력 게스처', () => {
    ...
    it('그라데이션 hex Escape는 시작 gradient를 cancel 채널로 넘기고 draft를 복원한다', () => {
      const onColorChange = vi.fn();
      const onColorChangeComplete = vi.fn();
      const onInputCancel = vi.fn();
      act(() =>
      ...
      act(() => setInputValue(top, '00FF00'));
      onColorChange.mockClear();
      act(() => pressKey(top, 'Escape'));

      expect(top.value).toBe('FF0000');
      expect(onInputCancel).toHaveBeenCalledWith('top', {
        type: 'gradient',
        top: '#FF0000',
        bottom: '#0000FF',
      });
    ...
    it('그라데이션의 유효 preview 뒤 invalid blur는 시작 gradient로 복원하고 확정하지 않는다', () => {
      const onColorChange = vi.fn();
      const onColorChangeComplete = vi.fn();
      act(() =>
        root.render(
    ...
    it('편집 중이 아닐 때 외부 solid color 변경을 hex 입력에 동기화한다', () => {
      const onColorChange = vi.fn();
      const onColorChangeComplete = vi.fn();
      ...
      act(() => render('#FF0000'));
      expect(hexInputs()[0].value).toBe('FF0000');

      act(() => render('#00FF00'));
      expect(hexInputs()[0].value).toBe('00FF00');
      expect(onColorChange).not.toHaveBeenCalled();
      expect(onColorChangeComplete).not.toHaveBeenCalled();
    });
