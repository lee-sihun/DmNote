...
// 핸들 시각적 스타일 반환
const getHandleStyle = (type: HandleDef['type'], isHovered: boolean): React.CSSProperties => {
...
// 개별 핸들 컴포넌트
function Handle({ handle, centerX, centerY, onMouseDown }: HandleProps): React.ReactElement {
...
export default function GroupResizeHandles({
  ...
  const handleMouseDown = (e: React.MouseEvent, handle: HandleDef) => {
      ...
      const getMaxShrink = (boundsList: ElementBounds[], groupSize: number, axis: 'x' | 'y'): number => {
      ...
      const handleMouseMove = (moveEvent: MouseEvent) => {
        ...
        const snapDelta = (delta: number): number => Math.round(delta / snapSize) * snapSize;

        const clampShrinkDelta = (delta: number, handleDir: -1 | 0 | 1, maxShrink: number): number => {
