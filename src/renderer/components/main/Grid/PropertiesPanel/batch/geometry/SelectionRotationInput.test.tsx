...
vi.mock('@components/main/Grid/PropertiesPanel/index', async () => {
  ...
  const { NumberInput } = await import(
    '@components/main/common/numberInput/NumberInput'
  );
  return {
    NumberInput,
...
const loadHarness = async () => {
  const [
    ...
    import('@hooks/Grid/selection/useSelectionRotationFrame'),
  ...
  const [input, boundary, scope, pending] = await Promise.all([
    ...
    import('../../selection/EditSessionBoundary'),
