/**
 * GraphItem trailing click 흡수 계약 테스트
...
import GraphItem from '@components/main/Grid/layers/GraphItem';
...
const GRAPH_ID = '00000000-0000-4000-8000-000000000601';
...
const renderGraph = (
  ...
  return container.querySelector('[data-graph-element="true"]') as HTMLElement;
};
...
describe('GraphItem trailing click 가드', () => {
  it('표식 없는 클릭은 onClick으로 통과한다', () => {
    const node = renderGraph();
  ...
  it('드래그 표식(wasMoved)이 선 클릭은 지우개·부모로 새지 않는다', () => {
    ...
    const node = renderGraph({ activeTool: 'eraser' });
  ...
  it('선택 모드: 표식 없는 수식키 클릭은 onCtrlClick으로 통과한다', () => {
    const node = renderGraph({ isSelected: true });
