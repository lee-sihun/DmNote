...
function buildLinePoints(history: number[], safeMax: number): { points: string; fillPoints: string } {
...
function buildBarPath(history: number[], safeMax: number, width: number, height: number): string {
...
const GraphPanel = forwardRef<HTMLDivElement, GraphPanelProps>(function GraphPanel(
  {
    dx = 0,
    dy = 0,
    width = 200,
  ...
  const { points: linePoints, fillPoints } = buildLinePoints(lineHistory, safeMax);
  const barHistory =
      ...
      {imageSrc ? (
        <img
          ...
          style={{
            ...
            width: '100%',
            height: '100%',
            objectFit: (imageFit || 'cover') as React.CSSProperties['objectFit'],
            pointerEvents: 'none',
            userSelect: 'none',
            zIndex: 0,
          }}
