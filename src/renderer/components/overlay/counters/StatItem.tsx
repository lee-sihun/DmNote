...
export default memo(function StatItem({
  ...
  const stateBorderColor: string | undefined = active
    ...
    : borderColor;
  const stateFontColor: string | undefined = active ? (activeFontColor ?? fontColor) : fontColor;
  ...
  const keyStyle: React.CSSProperties = useMemo(() => {
  ...
  const fallbackImageDimmed: boolean = active && !activeImageSrc && !!inactiveImageSrc;
