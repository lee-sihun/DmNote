...
const benchmarkDescribe =
  process.env.DMN_GRID_MINIMAP_BENCHMARK === '1' ? describe : describe.skip;
const OUTPUT_PATH =
  process.env.DMN_BENCHMARK_OUTPUT ??
  'benchmarks/results/grid-11-minimap-latest.json';
...
benchmarkDescribe('GRID-11 미니맵 드래그 성능', () => {
  ...
  it('mousemove burst의 event blocking과 DOM commit 분포를 기록한다', async () => {
    ...
    for (const itemCount of ITEM_COUNTS) {
      ...
      for (
        ...
        act(() => {
          useGridViewStore.setState((state) => ({
            viewStates: {
              ...state.viewStates,
              benchmark: { zoom: 1, panX: 0, panY: 0 },
            },
          }));
        });
      }
      ...
      `${JSON.stringify(
        {
          ...
          runtime: {
            kind: 'vitest-jsdom-minimap-drag-burst-proxy',
