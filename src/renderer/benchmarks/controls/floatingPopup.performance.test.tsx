...
const benchmarkDescribe =
  process.env.DMN_FLOATING_POPUP_BENCHMARK === '1' ? describe : describe.skip;
const OUTPUT_PATH =
  process.env.DMN_BENCHMARK_OUTPUT ??
  'benchmarks/results/base-08-floating-popup-latest.json';
...
const waitForPopupContent = (startedAt: number): Promise<number> => {
  if (document.querySelector('[data-benchmark-popup-content="true"]')) {
...
benchmarkDescribe('BASE-08 FloatingPopup 성능', () => {
  ...
  it('opener·shell 반영과 무거운 popup content mount 분포를 기록한다', async () => {
    ...
    for (const itemCount of ITEM_COUNTS) {
      ...
      for (let iteration = 0; iteration < totalIterations; iteration += 1) {
        ...
        const contentCommit = waitForPopupContent(startedAt);
        ...
        act(() => opener!.click());
        expect(
          document.querySelector('[data-benchmark-popup-content="true"]'),
        ).toBeNull();
      }
