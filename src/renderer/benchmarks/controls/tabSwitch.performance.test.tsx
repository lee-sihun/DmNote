...
const benchmarkDescribe =
  process.env.DMN_TAB_SWITCH_BENCHMARK === '1' ? describe : describe.skip;
const OUTPUT_PATH =
  process.env.DMN_BENCHMARK_OUTPUT ??
  'benchmarks/results/base-07-tab-switch-latest.json';
...
benchmarkDescribe('BASE-07 TabSwitch 성능', () => {
  ...
  it('탭 표시와 무거운 콘텐츠 교체의 DOM commit 분포를 기록한다', async () => {
    ...
    for (const itemCount of ITEM_COUNTS) {
      ...
      for (let iteration = 0; iteration < totalIterations; iteration += 1) {
        ...
        const current = canonicalHost!.getAttribute('data-canonical-tab');
        const expected = current === 'summary' ? 'details' : 'summary';
        const target = host.querySelector<HTMLElement>(
          `[data-tab-id="${expected}"]`,
        );
