...
vi.mock('@plugins/rpc/pluginElementActions', () => ({
  ...
  patchSoundPathViaAuthority: vi.fn(),
  patchSoundEnabledViaAuthority: vi.fn(),
  patchInactiveImageViaAuthority: vi.fn(),
  patchActiveImageViaAuthority: vi.fn(),
  patchIdleTransparentViaAuthority: vi.fn(),
  patchActiveTransparentViaAuthority: vi.fn(),
  patchCounterEnabledViaAuthority: vi.fn(),
  patchCounterAnimationEnabledViaAuthority: vi.fn(),
}));
...
describe('patchElementPropertyViaAuthority', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(patchSoundPathViaAuthority).mockResolvedValue(true);
    vi.mocked(patchSoundEnabledViaAuthority).mockResolvedValue(true);
    useKeyStore.setState({
      selectedKeyType: '4key',
      keyMappings: { '4key': ['A', 'B'] },
      canonicalPositions: { '4key': [keyAt(ID_A), keyAt(ID_B)] },
  ...
  // 드래그 가능한 토글·이미지 컨트롤이 쓰는 속성들. 하나라도 RPC 전용으로 남으면
  // 분리 창에서 값이 되돌아갔다가 왕복 뒤 다시 뒤집힌다
  it.each([
    [{ property: 'idleTransparent', value: true }, 'idleTransparent', true],
    [{ property: 'activeTransparent', value: true }, 'activeTransparent', true],
    [
      { property: 'inactiveImage', value: 'idle.png' },
      'inactiveImage',
      'idle.png',
    ],
    [
      { property: 'activeImage', value: 'active.png' },
      'activeImage',
      'active.png',
    ],
    [{ property: 'idleImageFit', value: 'fill' }, 'idleImageFit', 'fill'],
    [{ property: 'activeImageFit', value: 'none' }, 'activeImageFit', 'none'],
  ] as const)(
    '%o는 RPC 왕복 전에 반영하고 같은 patch를 authority로 보낸다',
    async (patch, field, expected) => {
      ...
      const pending = patchElementPropertyViaAuthority('key', ID_A, patch);

      const position = useKeyStore.getState().positions['4key'][0] as Record<
        string,
        unknown
      >;
      expect(position[field]).toBe(expected);
      expect(patchNativeLayerPropertyViaAuthority).toHaveBeenCalledWith({
        elementType: 'key',
        id: ID_A,
        patch,
      });
  ...
  // 사운드 두 속성만 재시도 정책이 달라 전용 sender로 나간다.
  // 즉시 반영은 나머지와 같은 프롤로그를 탄다
  it('soundEnabled는 즉시 반영 뒤 전용 sender로 나간다', async () => {
    await expect(
      patchElementPropertyViaAuthority('key', ID_A, {
        property: 'soundEnabled',
        value: true,
      }),
    ).resolves.toBe(true);

    expect(useKeyStore.getState().positions['4key'][0].soundEnabled).toBe(true);
    expect(patchSoundEnabledViaAuthority).toHaveBeenCalledWith([ID_A], true);
    expect(patchNativeLayerPropertyViaAuthority).not.toHaveBeenCalled();
  });

  it('soundPath는 즉시 반영 뒤 전용 sender로 나간다', async () => {
    await expect(
      patchElementPropertyViaAuthority('key', ID_A, {
        property: 'soundPath',
        value: 'sounds/tap.wav',
      }),
    ).resolves.toBe(true);

    expect(useKeyStore.getState().positions['4key'][0].soundPath).toBe(
      'sounds/tap.wav',
    );
    expect(patchSoundPathViaAuthority).toHaveBeenCalledWith(
      [ID_A],
      'sounds/tap.wav',
    );
    expect(patchNativeLayerPropertyViaAuthority).not.toHaveBeenCalled();
  ...
  it('사운드 전용 sender가 거절하면 즉시 반영을 되돌린다', async () => {
    vi.mocked(patchSoundEnabledViaAuthority).mockResolvedValue(false);
    ...
    expect(useKeyStore.getState().positions['4key'][0].soundEnabled).not.toBe(
      true,
    );
  });
...
// 배치 토글도 단건과 같은 즉시 반영을 거쳐야 왕복 동안 값이 되돌아가지 않는다.
// RPC는 배치 한 건으로 나가야 메인 쪽 undo 묶음이 쪼개지지 않는다
describe('patchElementPropertyByTargetsViaAuthority', () => {
  const targets = [
    { elementType: 'key' as const, id: ID_A },
  ...
  it('왕복 전에 모든 대상에 반영하고 배치 하나로 보낸다', async () => {
    let resolveRpc!: (persisted: boolean) => void;
    vi.mocked(patchIdleTransparentViaAuthority).mockImplementation(
      () =>
        new Promise((resolve) => {
          resolveRpc = resolve;
        }),
    ...
    const pending = patchElementPropertyByTargetsViaAuthority(targets, {
      property: 'idleTransparent',
      value: true,
    });

    const positions = useKeyStore.getState().positions['4key'];
    expect(positions[0].idleTransparent).toBe(true);
    expect(positions[1].idleTransparent).toBe(true);
    expect(patchIdleTransparentViaAuthority).toHaveBeenCalledOnce();
    expect(patchIdleTransparentViaAuthority).toHaveBeenCalledWith(
      targets,
      true,
    );
  ...
  it('authority가 거절하면 모든 대상의 즉시 반영을 되돌린다', async () => {
    vi.mocked(patchActiveTransparentViaAuthority).mockResolvedValue(false);

    await patchElementPropertyByTargetsViaAuthority(targets, {
      property: 'activeTransparent',
      value: true,
    });

    const positions = useKeyStore.getState().positions['4key'];
    expect(positions[0].activeTransparent).not.toBe(true);
    expect(positions[1].activeTransparent).not.toBe(true);
  });

  it('속성별 다건 sender로 정확히 갈라진다', async () => {
    vi.mocked(patchInactiveImageViaAuthority).mockResolvedValue(true);
    vi.mocked(patchActiveImageViaAuthority).mockResolvedValue(true);
    vi.mocked(patchSoundPathViaAuthority).mockResolvedValue(true);
    vi.mocked(patchSoundEnabledViaAuthority).mockResolvedValue(true);
    ...
    expect(patchInactiveImageViaAuthority).toHaveBeenCalledWith(
      targets,
      'idle.png',
    );
    expect(patchActiveImageViaAuthority).toHaveBeenCalledWith(
      targets,
      'active.png',
    );
    // 사운드 sender는 id 목록만 받는다
    expect(patchSoundPathViaAuthority).toHaveBeenCalledWith(
      [ID_A, ID_B],
      'tap.wav',
    );
    expect(patchSoundEnabledViaAuthority).toHaveBeenCalledWith(
      [ID_A, ID_B],
      true,
    );
    expect(patchNativeLayerPropertyViaAuthority).not.toHaveBeenCalled();
  ...
  it('다건 sender가 없는 속성과 불안정 대상은 wire 전에 거절한다', async () => {
    await expect(
    ...
    expect(patchIdleTransparentViaAuthority).not.toHaveBeenCalled();
    expect(useKeyStore.getState().positions['4key'][0].idleTransparent).toBe(
      false,
    );
  });

  // counter는 중첩 객체라 leaf 투영을 못 쓴다. 도킹 경로와 같은 intents 빌더를 탄다
  it('counter bool은 중첩 counter를 즉시 반영하고 전용 sender로 보낸다', async () => {
    vi.mocked(patchCounterEnabledViaAuthority).mockResolvedValue(true);
    vi.mocked(patchCounterAnimationEnabledViaAuthority).mockResolvedValue(true);

    await expect(
      patchCounterBooleanByTargetsViaAuthority(targets, {
    ...
    const position = useKeyStore.getState().positions['4key'][0];
    expect(position.counter?.enabled).toBe(false);
    expect(position.counter?.animation.enabled).toBe(true);
    expect(patchCounterEnabledViaAuthority).toHaveBeenCalledWith(
      targets,
      false,
    );
    expect(patchCounterAnimationEnabledViaAuthority).toHaveBeenCalledWith(
      targets,
      true,
    );
  });

  it('counter bool도 거절되면 즉시 반영을 되돌린다', async () => {
    vi.mocked(patchCounterEnabledViaAuthority).mockResolvedValue(false);

    await patchCounterBooleanByTargetsViaAuthority(targets, {
      property: 'counterEnabled',
      value: false,
    });

    expect(
      useKeyStore.getState().positions['4key'][0].counter?.enabled,
    ).not.toBe(false);
  });
});
