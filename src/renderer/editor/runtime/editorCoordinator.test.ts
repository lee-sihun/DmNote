...
describe('commitSemanticOpsInternal', () => {
  ...
  it('counter stroke descriptor projection은 gradient 형제를 갱신·제거한다', async () => {
    const id = '00000000-0000-4000-8000-0000000000cf';
    const base = withStableId(id);
    const gradient = {
      angle: 90,
      stops: [
        { color: 'rgba(170,187,204,1)', pos: 0 },
        { color: 'rgba(0,0,0,0.5)', pos: 1 },
      ],
    };
    base.keyPositions['4key'][0] = {
      ...base.keyPositions['4key'][0],
    ...
    await harness.coordinator.commitSemanticOpsInternal([
      {
        ...
        patch: {
          ...
          value: { color: 'rgba(170,187,204,1)', gradient },
        },
      },
    ]);
    const counter =
    ...
    expect(counter).toMatchObject({
      stroke: {
        idle: 'rgba(170,187,204,1)',
        active: 'old-active',
        custom: 'keep',
      },
      strokeIdleGradient: gradient,
      strokeActiveGradient: gradient,
      customSentinel: 'keep-raw',
    });
    harness.coordinator.stop();
  ...
  it('counter stroke solid descriptor는 gradient 형제를 제거한다', async () => {
    ...
    expect(
      (counter as unknown as Record<string, unknown>).strokeActiveGradient,
    ).toBeUndefined();
    harness.coordinator.stop();
  });
