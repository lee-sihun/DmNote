...
export const COUNTER_BEZIER_PRESETS: CounterBezierPreset[] = [
  {
    id: "current",
    labelKey: "counterSetting.presetCurrent",
    fallbackLabel: "Current",
    bezier: [0.25, 0.46, 0.45, 0.94],
  },
  {
    id: "linear",
    labelKey: "counterSetting.presetLinear",
    fallbackLabel: "Linear",
    bezier: [0, 0, 1, 1],
  },
  {
    id: "easeOut",
    labelKey: "counterSetting.presetEaseOut",
    fallbackLabel: "Ease Out",
    bezier: [0, 0, 0.58, 1],
  },
  {
    id: "easeIn",
    labelKey: "counterSetting.presetEaseIn",
    fallbackLabel: "Ease In",
    bezier: [0.42, 0, 1, 1],
  },
  {
    id: "easeInOut",
    labelKey: "counterSetting.presetEaseInOut",
    fallbackLabel: "Ease In-Out",
    bezier: [0.42, 0, 0.58, 1],
  },
  {
    id: "overshoot",
    labelKey: "counterSetting.presetOvershoot",
...
export const clampCounterBezier = (
  ...
  return [
    ...
    Math.min(Math.max(Number(bezier?.[1] ?? COUNTER_DEFAULT_BEZIER[1]), -2), 2),
    Math.min(Math.max(Number(bezier?.[2] ?? COUNTER_DEFAULT_BEZIER[2]), 0), 1),
    Math.min(Math.max(Number(bezier?.[3] ?? COUNTER_DEFAULT_BEZIER[3]), -2), 2),
  ];
};
...
export const createCubicBezierEasing = (
  ...
  const [x1, y1, x2, y2] = clampCounterBezier(
    bezier ?? COUNTER_DEFAULT_BEZIER,
  );
