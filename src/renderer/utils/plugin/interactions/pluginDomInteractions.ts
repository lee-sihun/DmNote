...
type PluginDomRoot = HTMLElement | ShadowRoot;

interface PluginDomInteractionOptions {
  requireCheckboxKnob?: boolean;
}

export const attachPluginDomInteractions = (
  root: PluginDomRoot,
  { requireCheckboxKnob = false }: PluginDomInteractionOptions = {},
) => {
  ...
  const handleCheckboxToggle = (event: Event) => {
    ...
    if (!input || (requireCheckboxKnob && !knob)) return;
