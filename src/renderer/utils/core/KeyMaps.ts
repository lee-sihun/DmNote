...
export const keyMaps: Record<string, KeyInfo> = {
  // 알파벳
  ...
  Backslash: { browserKey: '\\', globalKey: 'BACKSLASH', displayName: '\\' },
...
// 마우스 / 게임패드
const extraGlobalKeyMap: Record<string, KeyInfo> = {
  MOUSE1: {
...
const globalKeyLookup: Record<string, KeyInfo> = (() => {
  const map: Record<string, KeyInfo> = { ...extraGlobalKeyMap };
...
export const getKeyInfo = (code: string, key: string): KeyInfo => {
  return (
    keyMaps[code] || {
      browserKey: key,
...
export const getKeyInfoByGlobalKey = (globalKey: string): KeyInfo => {
