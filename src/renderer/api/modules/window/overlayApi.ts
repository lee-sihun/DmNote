...
export const overlayApi = {
  ...
  setVisible: (visible: boolean) =>
    invokeEditorWrite<void>('overlay_set_visible', { visible }),
  setLock: (locked: boolean) =>
    invokeEditorWrite<void>('overlay_set_lock', { locked }),
  setAnchor: (anchor: string) =>
    invokeEditorWrite<string>('overlay_set_anchor', { anchor }),
  ...
  }) => invokeEditorWrite<OverlayBounds>('overlay_resize', { payload }),
  // 오버레이를 작업 영역 안쪽 중앙으로 되돌린다 - 화면 밖으로 나갔을 때의 탈출구
  resetPosition: () =>
    invokeEditorWrite<OverlayBounds>('overlay_reset_position'),
