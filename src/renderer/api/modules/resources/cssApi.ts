...
export const cssApi = {
  ...
  toggle: (enabled: boolean) =>
    invokeEditorWrite<CssTogglePayload>('css_toggle', { enabled }),
  load: () => invokeEditorWrite<CssLoadResult>('css_load'),
  setContent: (content: string) =>
    invokeEditorWrite<CssSetContentResult>('css_set_content', { content }),
  reset: () => invokeEditorWrite<void>('css_reset'),
  historyGet: () => invoke<CustomCssHistoryItem[]>('css_history_get'),
  historyActivate: (path: string) =>
    invokeEditorWrite<CssActivateResult>('css_history_activate', { path }),
  historyRemove: (path: string) =>
    invokeEditorWrite<CustomCssHistoryItem[]>('css_history_remove', { path }),
  ...
  tab: {
    ...
    load: (tabId: string) =>
      invokeEditorWrite<import('@src/types/plugin/api').TabCssLoadResult>(
        'css_tab_load',
        {
          tabId,
        },
    ...
    clear: (tabId: string) =>
      invokeEditorWrite<import('@src/types/plugin/api').TabCssClearResult>(
        'css_tab_clear',
        {
          tabId,
        },
    ...
    toggle: (tabId: string, enabled: boolean) =>
      invokeEditorWrite<import('@src/types/plugin/api').TabCssToggleResult>(
        'css_tab_toggle',
        {
          tabId,
          enabled,
    ...
    activateHistory: (tabId: string, path: string) =>
      invokeEditorWrite<import('@src/types/plugin/api').TabCssActivateResult>(
    ...
    export: (tabId: string) =>
      invokeEditorWrite<import('@src/types/plugin/api').TabCssExportResult>(
        'css_tab_export',
        {
          tabId,
        },
    ...
    set: (tabId: string, css: import('@src/types/plugin/css').TabCss | null) =>
      invokeEditorWrite<import('@src/types/plugin/api').TabCssSetResult>(
        'css_tab_set',
        {
          tabId,
          css,
