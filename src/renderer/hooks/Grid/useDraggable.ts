...
const calculateDynamicGridSize = (zoom: number, baseGridSize: number): number => {
...
export const useDraggable = ({
  ...
  const [{ dx, dy }, setOffset] = useState<{ dx: number; dy: number }>({ dx: initialX, dy: initialY });

  // 마지막 스냅 좌표를 ref로 보관 (mouseup 시 커밋)
  const lastSnappedRef = useRef<{ dx: number; dy: number }>({ dx: initialX, dy: initialY });
  ...
  const getOtherElementsRef = useRef<((excludeId: string) => ElementBounds[]) | null>(getOtherElements);
