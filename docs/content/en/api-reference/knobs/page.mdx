...
type KnobItemPosition = KeyPosition & {
  ...
  sensitivity: number; // degrees of on-screen rotation per raw unit
...
`accumulatedDelta × sensitivity` degrees.
...
```typescript
const positions = await dmn.knobItems.getPositions();
positions['4key'][0].sensitivity = 2.8125; // 2 turns per knob revolution
