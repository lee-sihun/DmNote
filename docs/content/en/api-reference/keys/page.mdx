...
await dmn.keys.resetSingleCounter('4key', 'F|NUMPAD 4');
```
...
console.log('Mode changed:', result.success);
```

### `dmn.keys.resetMode(mode): Promise<KeysModeResponse>`

Resets one key mode and its positions to the built-in defaults.
...
Resets all keys, positions, counters, and custom tabs to their built-in defaults.

```javascript
const result = await dmn.keys.resetAll();
console.log('Selected mode:', result.selectedKeyType);
```
...
and `counter.fill.active`. Counter stroke follows the same pattern:
`counter.strokeIdleGradient` and `counter.strokeActiveGradient` take priority
over `counter.stroke.idle` and `counter.stroke.active`, and the solid fields
stay synced to the first stop color. Known limitation: a gradient stroke paints
the full glyph area behind the fill, so when the counter fill is
semi-transparent the stroke gradient also shows through the glyph interior.
Fully opaque fills render an outline-only ring as expected.
...
restrictions match the border. Writing a legacy color value or per-end opacity
patch removes the new field and returns the surface to legacy semantics. Use
the `{ color, opacity, gradient }` form of `notePaint`/`noteGlowPaint` to
record the transition, multiplier, and synced values atomically.
...
console.log(result.result?.id, result.error);
```
...
console.log('Selected mode:', result.selected);
```
...
Subscribes to custom tab create, delete, select, and restore changes.

```javascript
const unsub = dmn.keys.customTabs.onChanged((payload) => {
  console.log(payload.customTabs, payload.selectedKeyType);
});
```
