...
to recover the canonical input timeline after its event subscription becomes
ready or a revision gap is detected. This command is not part of `dmn.keys`, is
not available through the OBS invoke allowlist, and must not be used by
plugins. OBS performs the equivalent recovery through protocol v3
`timeline_replay` and `timeline_rebase` messages.
