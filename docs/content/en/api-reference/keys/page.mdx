...
Resets one key mode, its positions, and plugin display elements placed in that
mode to the built-in defaults. Other plugin storage is preserved.
...
The name is trimmed before validation. It is rejected when empty
...
Subscribes to custom tab create, delete, select, restore, rename, and reorder
changes.

Alongside `customTabs` and `selectedKeyType`, the payload carries `tabOrder` and
`barCount`. `tabOrder` is the combined display order of built-in modes and custom
tabs, containing every tab ID exactly once; `barCount` is how many of the leading
entries appear in the toolbar bar (1 to 4). Since `list()` returns only an array,
read the display order from `tabOrder`.
