...
interface ReactiveSpritePosition {
  ...
  imagePlacement: 'box' | 'pivot'; // image placement, default box. pivot aligns each image's own anchor to the pivot point and draws it at the reference image's pixel scale
  referenceNaturalSize: { source: string | null; width: number; height: number } | null; // pixel-scale reference for pivot placement. source must equal baseImage when one is set, and be null otherwise
}
...
interface SpritePose {
  ...
  imagePivot: { x: number; y: number } | null; // anchor of this pose image under pivot placement, 0–1 each. null inherits the sprite pivot; normalized to null when imageOverride is absent
  imageOverrideMetrics: { source: string; width: number; height: number } | null; // intrinsic pixel size of imageOverride (integers, 1–32768). Required under pivot placement when an override is set, and source must equal the path exactly
}
...
Under `pivot` placement every image puts its own anchor on the pivot point P
(imageRect origin + pivot × size) and is drawn at intrinsic pixels × the
reference scale. The reference scale divides imageRect by referenceNaturalSize:
contain takes the smaller ratio, cover the larger, fill uses each axis. When
poses blend, anchors are never averaged; the pose whose image was chosen supplies
its anchor. Under pivot placement a missing or mismatched intrinsic size rejects
the commit with `VALIDATION_FAILED` and a `details.validationCode` of
`MISSING_SPRITE_IMAGE_METRICS`, `STALE_SPRITE_IMAGE_METRICS`, or
`INVALID_SPRITE_IMAGE_METRICS`. A schemaVersion 1 patch that omits
`imagePlacement`, `referenceNaturalSize`, `imagePivot`, `imageOverrideMetrics`,
`contactPoint`, `activation`, or `pressDurationMs` on an existing element or
pose keeps the stored value.
