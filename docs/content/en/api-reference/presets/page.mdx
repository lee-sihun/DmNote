...
dialog. Element collections stored in the file (keys, stats, graphs, knobs,
sprites) replace the tab's contents wholesale, so elements missing from the
file's tab are removed. Older presets that lack a collection entirely leave
