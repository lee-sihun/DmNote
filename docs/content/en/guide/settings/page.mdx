...
When enabled, the mouse is no longer captured even on keys, so the overlay cannot be moved or right-clicked. Transparent areas outside keys always pass clicks through to the window underneath regardless of this setting.
