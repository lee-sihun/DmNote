...
- Stylesheets pulled in with `@import` are fetched by the app and confined to
  ...
  main window only.
- `@font-face` and `@counter-style` register for the whole document and pass
  through unchanged, so avoid `font-family` names that the app UI uses.
  `@property` can collide with names the app registers and is ignored in the
  main window.
