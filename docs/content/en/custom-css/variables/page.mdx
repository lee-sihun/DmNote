...
| `--key-bg-image`   | Background image (gradient) | `<image>` | `linear-gradient(90deg, #f0f, #0ff)` |
...
<Callout type="info">
  ...
  if needed. When a key color is set to a gradient in the app, it renders as
  `background-image: var(--key-bg-image, ...)`, so you can override it with
  `--key-bg-image` (solid keys never get an inline `background-image`).
</Callout>
