...
`type: "section"`은 새 카드를 시작합니다. section 키는 레이아웃 식별자이므로 저장값,
기본값, `getSettings()`, 변경 콜백에 포함되지 않습니다. 첫 section 앞의 설정은 암시적
무제목 카드에 표시되며, `label`이 없어도 카드 경계로 동작합니다.

<Callout type="info">
  과거의 `type: "divider"`(카드 안 구분선)는 section 도입과 함께 제거되었습니다.
  기존 플러그인의 divider 항목은 무시되며(크래시 없음), 그룹 구분은 section을
  사용하세요.
</Callout>

section은 속성 패널과 `settingsUI: "modal"`에서 동일하게 동작합니다.
...
section의 `visible`은 다음 section 전까지의 그룹 전체에 적용됩니다. 숨겨져도 경계는
유지되므로 인접 그룹이 합쳐지지 않습니다. section 안의 값 설정이 모두 숨겨지면 카드와
캡션도 함께 숨겨집니다. visibility 함수가 예외를 던지면 해당 항목 또는 section을
숨기는 fail-closed 방식으로 처리하고 오류를 기록합니다. 패널과 모달의 의미 규칙 및
빈 상태 동작은 같습니다.
...
`position: "top"` 항목은 기본 메뉴 항목 위에, 나머지는 아래에 표시됩니다.

<Callout type="info">
  메뉴 predicate는 메인 윈도우에서 `{ element, actions }`로 평가됩니다.
  오버레이에서 `setState`로 갱신한 런타임 상태는 기본적으로 이 컨텍스트에
  동기화되지 않으며, `element.state`는 메인 윈도우 상태(`previewState`로
  초기화)를 반영합니다. 오버레이 상태 기반 조건이 필요하면
  `contextMenuStateKeys`에 키를 선언하세요.
</Callout>

### predicate에서 오버레이 상태 사용 (contextMenuStateKeys)

메뉴 predicate가 참조할 오버레이 상태 키를 선언합니다. 선언된 키만
(`setState` 변경 시) 메인 윈도우로 미러링되어 predicate 평가 시
`element.state`에 병합됩니다. 고빈도 상태는 선언하지 않는 한 절대 전송되지
않고, 메인 윈도우의 프리뷰 상태도 변하지 않습니다.

```javascript fragment=object-members
// 메뉴 predicate용으로 active만 미러링
