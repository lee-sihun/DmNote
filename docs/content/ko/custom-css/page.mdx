...
- `@import`로 가져온 스타일시트는 앱이 직접 받아 온 뒤 같은 방식으로 미리보기
  ...
  4단계 이상 중첩)는 메인 화면에서만 건너뜁니다.
- `@font-face`와 `@counter-style`은 문서 전체에 등록되어 그대로 통과합니다. 앱 UI가
  쓰는 글꼴 이름과 같은 `font-family`는 피하세요. `@property`는 앱이 등록한 이름과
  충돌할 수 있어 메인 화면에서는 무시됩니다.
