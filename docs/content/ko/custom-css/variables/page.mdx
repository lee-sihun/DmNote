...
`--sprite-transform`은 이미지의 로컬 자세 변환만 교체합니다. 요소 배치용
`rotation`은 앱이 상자 중심을 축으로 적용하며 자세 변환 바깥에서 함께 돕니다.
...
이미지 요소는 원본 비율 그대로 상자를 채우므로 `object-fit`이 바꿀 것이 없습니다.
크기 자체를 CSS로 덮으면 자세 이미지의 기준점 정합은 보장하지 않습니다.
...
| 선택자                                    | 설명                                                  |
| ----------------------------------------- | ----------------------------------------------------- |
| `[data-sprite-element]`                   | 모든 스프라이트 요소 상자                             |
