...
| `--key-bg-image`   | 키의 배경 이미지(그라데이션) | `<image>` | `linear-gradient(90deg, #f0f, #0ff)` |
...
<Callout type="info">
  ...
  transparent;`로 설정하세요. 앱에서 키 색상을 그라데이션으로 설정한 경우에는
  `background-image: var(--key-bg-image, ...)`로 렌더되므로 `--key-bg-image`로
  덮어쓸 수 있습니다 (단색 키에는 인라인 `background-image`가 붙지 않습니다).
</Callout>
