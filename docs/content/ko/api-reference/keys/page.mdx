...
우선합니다. 카운터 stroke도 같은 구조입니다. `counter.strokeIdleGradient`와
`counter.strokeActiveGradient`가 각각 `counter.stroke.idle`,
`counter.stroke.active`보다 우선하며, 단색 필드는 첫 스톱 색으로 동기화됩니다.
알려진 제약: 그라데이션 stroke는 글리프 전체를 fill 뒤에서 칠하는 구조라
카운터 fill이 반투명이면 글자 내부로도 비칩니다. 불투명 fill에서는 기대대로
외곽선만 보입니다.
...
동기됩니다. 스톱 색 형식 제한은 테두리와 같습니다. 구형 색 값이나 끝단
투명도 patch를 쓰면 신형 필드가 제거되고 구형 의미로 되돌아갑니다.
`notePaint`/`noteGlowPaint`에 `{ color, opacity, gradient }` 형태를 쓰면
전환·배율·동기값을 한 번에 원자적으로 기록합니다.
