...
const pluginSettings = dmn.plugin.defineSettings({
  settings: {
    ...
    // 조건부 표시. 정적 boolean 또는 함수, enabled가 true일 때만 표시
    advancedOption: {
      type: "number",
      default: 5,
      label: "고급 옵션",
...
// section은 새 카드를 시작합니다. (과거의 divider 타입은 제거됨, 기존 플러그인의 divider는 무시)
...
`defineSettings`는 `defineElement`와 같은 section 계약을 사용합니다. section은 새 카드를
시작하고 선택적 label은 카드 위에 표시되며, section 키는 저장값·기본값·콜백에서
제외됩니다. `section.visible`은 그룹 전체를 제어합니다. 렌더 가능한 값 설정이 없는
section은 카드째 숨겨지고, 전체에 렌더 가능한 값 설정이 하나도 없을 때만 표준 빈
상태를 표시합니다. 패널과 모달은 visibility 예외의 fail-closed 처리까지 같은 의미로
동작합니다.
...
`set()`과 `reset()`은 저장이 실패하면 Promise를 거부합니다. 실패한 편집은 저장된
설정으로 복원하며, 복원 중 시작한 새 편집은 덮지 않습니다. 변경 콜백과 구독 알림은
저장 성공 후에만 전달됩니다. 속성 패널의 저장 실패는 안내 후 `open()`이 `false`로
끝나며, 모달 방식의 저장 실패는 `open()` 호출자에게 오류로 전달됩니다.
