...
- main의 `npm version` 커밋: 운영 인증서 활성화 전까지 SignPath 단계를 건너뛰고 미서명 EXE/ZIP을 동일 draft 릴리즈에 업로드
- `workflow_dispatch(release=true)`: 현재 버전을 수동으로 동일 draft 릴리즈에 업로드
- `workflow_dispatch(release=false)`: SignPath 자체 서명 테스트 인증서를 적용하고 Actions artifact로만 업로드
...
배포 자산은 `distribution/windows/assets/`를 재귀적으로 그대로 포함한다. 이후 자산을 추가하거나 제거할 때는 이 폴더만 수정하면 ZIP 구성이 자동으로 따라간다. `THIRD_PARTY_NOTICES.txt`는 현재 바이너리 의존성과 맞아야 하므로 저장소 최신본을 별도로 포함한다.

## 2. 현재 버전 커밋 릴리즈 (운영 인증서 활성화 전)
...
`package.json` 버전 변경과 커밋 제목 `X.Y.Z`가 일치하는 main push는 macOS와 Windows workflow를 함께 시작한다. 사용자가 로컬 `X.Y.Z` 태그를 push할 필요는 없으며, 원격 태그와 draft 릴리즈는 Actions가 자동 생성한다. Windows workflow는 버전 일치를 검사하고 EXE를 빌드하지만 SignPath 제출·서명·서명 검증 단계는 실행하지 않는다. 이후 다음 자산을 macOS workflow와 동일한 draft 릴리즈에 생성하거나 덮어쓴다.

- `DM.NOTE.exe`
- `DM.NOTE.v.X.Y.Z.zip`

이미 publish된 릴리즈는 자동으로 덮어쓰지 않는다. 운영 인증서 활성화 전 Windows 자산은 미서명이다. 릴리즈 본문에는 이 사실을 따로 표기하지 않는다. 테스트 인증서로 서명된 파일을 대신 배포하지 않는다.
...
2. Actions -> **Release Windows** -> **Run workflow**를 기본 브랜치에서 `release=false`로 실행한다.
