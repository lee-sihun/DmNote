...
| 3 | **Stats (KPS) 동기화** | KPS 값을 OBS 클라이언트에 실시간 전송 (stats broadcast 또는 counter_update 확장) | ❌ |
| 4 | **UI 안내 문구** | OBS 설정 방법 가이드 텍스트 + 오버레이↔OBS 모드 전환 라디오 버튼 | ❌ |
...
| 7 | **keyDisplayDelayMs** | OBS에서 키 표시 지연 반영 | ❌ |
