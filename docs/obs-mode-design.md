...
> 백엔드 `register_event_forwarding()`이 32개 Tauri 이벤트를 자동 포워딩.
...
- `register_event_forwarding(app)` — 32개 Tauri 이벤트 → WS 자동 포워딩 ✅
...
|---------------|---------------|------|
| `tauri_event` | 이벤트명 그대로 | 범용 이벤트 포워딩 — 32개 이벤트 자동 디스패치 |
...
| 3 | **백엔드 이벤트 포워딩** — 32개 Tauri 이벤트 → `tauri_event` WS 포워딩 | `obs_bridge.rs`, `app_state.rs` | ✅ `28adb94` |
