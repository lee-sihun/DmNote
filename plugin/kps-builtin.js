// @id kps-builtin
// dmn.stats API를 사용하는 KPS 패널 (내장 통계 기반)

dmn.plugin.defineElement({
  name: "KPS (Built-in Stats)",
  maxInstances: 1,
  resizable: true,
  preserveAxis: "width",
  ...
  settings: {
    ...
    showTotal: { type: "boolean", default: false, label: "settings.showTotal" },
    showGraph: {
      type: "boolean",
      default: true,
  ...
  onMount: ({ setState, getSettings, expose, setAnchor, onSettingsChange }) => {
    let historyBuffer = [];
    ...
    historyBuffer = new Array(initialTargetSize).fill(0);

    const uid = Math.random().toString(36).substr(2, 9);
    setState({ uid, history: [...historyBuffer] });

    // 설정 변경 시 그래프 버퍼 크기 조정
    onSettingsChange((newSettings, oldSettings) => {
      // graphSpeed 변경 시 버퍼 크기 조정
      if (newSettings.graphSpeed !== oldSettings.graphSpeed) {
        const targetSize = Math.ceil(
          (newSettings.graphSpeed || 1000) / GRAPH_UPDATE_MS,
        );
    ...
    // dmn.stats 구독 (동기 - 즉시 unsubscribe 함수 반환)
    unsubscribe = dmn.stats.subscribe(({ kps, kpsAvg, kpsMax, total }) => {
      // 히스토리 업데이트 (50ms마다 수신됨)
      historyBuffer.shift();
      ...
      // 그래프 스케일링용 최대값
      const maxval = Math.max(...historyBuffer, 1);
