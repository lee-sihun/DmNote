...
const variants = [
  [
    'legacy',
    'legacy-baseline',
    'benchmarks/results/grid-08-gradient-axis-baseline.json',
  ],
  [
    'frame',
    'improved',
    'benchmarks/results/grid-08-gradient-axis-improved.json',
  ],
];
...
for (const [strategy, variant, output] of variants) {
  const result = spawnSync(
    npm,
    ['run', 'benchmark:interaction:gradient-axis:raw'],
    {
      cwd: root,
      env: {
        ...process.env,
...
if (!beforeCase || !afterCase) throw new Error('GRID-08 측정 case 누락');
...
if (gain === null || gain <= 0) {
  throw new Error(`GRID-08 event P95 미개선: ${ms(before)}ms → ${ms(after)}ms`);
}
...
const resultBlock = `<!-- GRID-08:RESULT:START -->
#### GRID-08 그라데이션 축 최신 자동 측정

| 조건 | 값 |
| --- | --- |
| 측정 경로 | 실제 GradientAxisOverlay + preview 구독 요소 ${
...
const sessionRow = (id, stage, result, data, output) =>
  `| ${id} | ${date} | GRID-08 | ${stage} | \`${result.commit.slice(
    0,
    8,
  )}\` | ${runtime} | preview 구독 ${data.itemCount}개·pointermove ${
    data.burstSize
  ...
  )}ms | [JSON](../${output}) | gradient stop burst proxy |`;
const sessionsBlock = `<!-- GRID-08:SESSIONS:START -->
| 세션 ID | 날짜 | 항목 ID | 단계 | 빌드·커밋 | 환경 | 시나리오·데이터 크기 | 반복 | P50 | P95 | 최대 | 보조 지표 | 원시 자료 | 비고 |
| --- | --- | --- | --- | --- | --- | --- | ---: | ---: | ---: | ---: | --- | --- | --- |
${sessionRow('GRID-08-LEGACY', '기준선', baseline, beforeCase, variants[0][2])}
${sessionRow('GRID-08-FRAME', '개선', improved, afterCase, variants[1][2])}
<!-- GRID-08:SESSIONS:END -->`;

const experimentBlock = `<!-- GRID-08:EXPERIMENT:START -->
...
tracker = tracker.replace(
  /^\| GRID-08\s+\|.*$/m,
  `| GRID-08 | 그라데이션 축 핸들 | P0 | F95 ms/frame | ${ms(before)} | ${ms(
    after,
  )} | ${pct(gain)} | 검증 | \`${short}\`, latest pointer frame coalescing |`,
);
tracker = tracker.replace(
  /^\| EDIT-03\s+\|.*$/m,
  `| EDIT-03 | 그라데이션 stop 편집·형식 전환 | P0 | F95 ms/frame | ${ms(
    before,
  )} | ${ms(after)} | ${pct(
    gain,
  )} | 검증 | \`${short}\`, stop 드래그 병합·형식 탭 즉시 반영 |`,
);
...
console.info(
  `GRID-08 event P95: ${ms(before)}ms → ${ms(after)}ms (${pct(gain)})`,
);
