...
const variants = [
  [
    'legacy',
    'legacy-baseline',
    'benchmarks/results/edit-01-color-track-baseline.json',
  ],
  ['frame', 'improved', 'benchmarks/results/edit-01-color-track-improved.json'],
];
for (const [strategy, variant, output] of variants) {
  const result = spawnSync(
    npm,
    ['run', 'benchmark:interaction:color-track:raw'],
    {
      cwd: root,
      env: {
        ...process.env,
...
if (!beforeCase || !afterCase) throw new Error('EDIT-01 측정 case 누락');
...
if (gain === null || gain <= 0) {
  throw new Error(`EDIT-01 event P95 미개선: ${ms(before)}ms → ${ms(after)}ms`);
}
const correctness = spawnSync(
  ...
  [
    'vitest',
    'run',
    'src/renderer/components/main/Modal/content/pickers/colorPickerPrimitives.test.tsx',
...
const implementation = execFileSync(
  ...
  [
    ...
    '--format=%H',
    '--',
    'src/renderer/components/main/Modal/content/pickers/colorPickerPrimitives.tsx',
  ],
...
const resultBlock = `<!-- EDIT-01:RESULT:START -->
#### EDIT-01 색상 트랙 최신 자동 측정

| 조건 | 값 |
| --- | --- |
| 측정 경로 | 실제 SaturationArea + 렌더 DOM ${
  beforeCase.itemCount
}개, pointermove ${beforeCase.burstSize}회 burst |
...
const row = (id, stage, result, data, output) =>
  `| ${id} | ${date} | EDIT-01 | ${stage} | \`${result.commit.slice(
    0,
    8,
  )}\` | ${runtime} | DOM ${data.itemCount}개·pointermove ${
    data.burstSize
  }회 | ${data.iterations} | ${ms(data.eventBlockingMs.p50)} | ${ms(
    data.eventBlockingMs.p95,
  )} | ${ms(data.eventBlockingMs.max)} | DOM P95 ${ms(
    data.visualDomCommitMs.p95,
  )}ms·React P95 ${ms(
    data.reactCommitDurationMs.p95,
  )}ms | [JSON](../${output}) | color track proxy |`;
const sessionsBlock = `<!-- EDIT-01:SESSIONS:START -->
| 세션 ID | 날짜 | 항목 ID | 단계 | 빌드·커밋 | 환경 | 시나리오·데이터 크기 | 반복 | P50 | P95 | 최대 | 보조 지표 | 원시 자료 | 비고 |
| --- | --- | --- | --- | --- | --- | --- | ---: | ---: | ---: | ---: | --- | --- | --- |
${row('EDIT-01-LEGACY', '기준선', baseline, beforeCase, variants[0][2])}
${row('EDIT-01-FRAME', '개선', improved, afterCase, variants[1][2])}
<!-- EDIT-01:SESSIONS:END -->`;
const experimentBlock = `<!-- EDIT-01:EXPERIMENT:START -->
...
const replacement = (id, label) =>
  `| ${id} | ${label} | P0 | F95 ms/frame | ${ms(before)} | ${ms(
    after,
  )} | ${pct(gain)} | 검증 | \`${short}\`, 공통 pointer frame coalescing |`;
tracker = tracker
  .replace(
    /^\| EDIT-01\s+\|.*$/m,
    replacement('EDIT-01', '색상 saturation·hue·alpha 드래그'),
  )
  .replace(/^\| PROP-03\s+\|.*$/m, replacement('PROP-03', '색상 입력'))
  .replace(/^\| PROP-04\s+\|.*$/m, replacement('PROP-04', '그라데이션 입력'))
  .replace(
    /^\| PROP-11\s+\|.*$/m,
    `| PROP-11 | 플러그인 설정 color | P0/P1 | F95 ms/frame | ${ms(
      before,
    )} | ${ms(after)} | ${pct(
      gain,
    )} | 검증 | \`${short}\`, 공통 ColorInput pointer 병합 |`,
  );
...
console.info(
  `EDIT-01 event P95: ${ms(before)}ms → ${ms(after)}ms (${pct(gain)})`,
);
