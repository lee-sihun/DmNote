...
const variants = [
  {
    ...
    output: 'benchmarks/results/base-06-color-input-baseline.json',
  },
  {
    strategy: 'after-paint',
    variant: 'improved',
    output: 'benchmarks/results/base-06-color-input-improved.json',
  },
];
...
for (const entry of variants) {
  const result = spawnSync(
    npmCommand,
    ['run', 'benchmark:interaction:color-input:raw'],
    {
      cwd: root,
      env: {
        ...process.env,
...
if (!baselineCase || !improvedCase) {
  throw new Error('BASE-06 benchmark 결과에 비교할 case가 없습니다.');
}
...
const pickerBefore = baselineCase.pickerDomCommitMs.p95;
const pickerAfter = improvedCase.pickerDomCommitMs.p95;
...
if (visualImprovement === null || visualImprovement <= 0) {
  throw new Error(
    `BASE-06 스와치 DOM commit P95가 개선되지 않았습니다: ${formatMs(
      visualBefore,
...
const resultBlock = `<!-- BASE-06:RESULT:START -->
#### BASE-06 ColorInput·ColorSwatchButton 최신 자동 측정

| 조건 | 값 |
| --- | --- |
| 측정 경로 | 공통 ColorInput + 피커 DOM ${
...
| --- | ---: | ---: | ---: |
| 스와치 열림 DOM commit | ${formatMs(visualBefore)}ms | ${formatMs(
  visualAfter,
)}ms | ${formatPercent(visualImprovement)} |
| 피커 mount DOM commit | ${formatMs(pickerBefore)}ms | ${formatMs(
  pickerAfter,
)}ms | ${formatPercent(improvement(pickerBefore, pickerAfter))} |
...
const sessionRow = (id, stage, result, resultCase, output) =>
  `| ${id} | ${measuredDate} | BASE-06 | ${stage} | \`${result.commit.slice(
    0,
    8,
  )}\` | ${runtimeLabel} | 피커 DOM ${resultCase.itemCount}개 | ${
    resultCase.iterations
  } | ${formatMs(resultCase.visualDomCommitMs.p50)} | ${formatMs(
    resultCase.visualDomCommitMs.p95,
  )} | ${formatMs(resultCase.visualDomCommitMs.max)} | picker P95 ${formatMs(
    resultCase.pickerDomCommitMs.p95,
  )}ms·event P95 ${formatMs(
    resultCase.eventBlockingMs.p95,
  )}ms | [JSON](../${output}) | DOM commit proxy |`;
const sessionsBlock = `<!-- BASE-06:SESSIONS:START -->
...
const experimentBlock = `<!-- BASE-06:EXPERIMENT:START -->
...
tracker = tracker.replace(
  /^\| BASE-06\s+\|.*$/m,
  `| BASE-06 | ColorInput·ColorSwatchButton | P0/P1 | DOM P95 ms | ${formatMs(
    visualBefore,
...
tracker = tracker.replace(
  /^\| PROP-11\s+\|.*$/m,
  `| PROP-11 | 플러그인 설정 color | P0/P1 | F95 ms/frame | — | — | — | 실험 | \`${implementationShort}\`, 공통 ColorInput 피커 mount 분리 적용 |`,
);
tracker = tracker.replace(
  /^\| PROP-03\s+\|.*$/m,
  `| PROP-03 | 색상 입력 | P0 | F95 ms/frame | — | — | — | 실험 | \`ad22c019\` 상태 탭·\`${implementationShort}\` 피커 mount 분리 적용 |`,
);
...
console.info(
  `BASE-06 swatch DOM commit P95: ${formatMs(visualBefore)}ms → ${formatMs(
    visualAfter,
  )}ms (${formatPercent(visualImprovement)})`,
);
