...
const variants = [
  {
    ...
    output: 'benchmarks/results/base-04-number-input-baseline.json',
  },
  {
    strategy: 'after-paint',
    variant: 'improved',
    output: 'benchmarks/results/base-04-number-input-improved.json',
  },
];
...
for (const entry of variants) {
  const result = spawnSync(
    npmCommand,
    ['run', 'benchmark:interaction:number-input:raw'],
    {
      cwd: root,
      env: {
        ...process.env,
...
if (!baselineCase || !improvedCase) {
  throw new Error('BASE-04 benchmark 결과에 비교할 case가 없습니다.');
}
if (baselineCase.itemCount !== improvedCase.itemCount) {
  throw new Error('BASE-04 baseline과 improved의 항목 수가 다릅니다.');
}
...
if (visualImprovement === null || visualImprovement <= 0) {
  throw new Error(
    `BASE-04 시각 DOM commit P95가 개선되지 않았습니다: ${formatMs(
      visualBefore,
...
const resultBlock = `<!-- BASE-04:RESULT:START -->
#### BASE-04 NumberInput 최신 자동 측정

| 조건 | 값 |
| --- | --- |
| 측정 경로 | 공통 NumberInput + 부모 콘텐츠 DOM ${
...
const sessionRow = (id, stage, result, resultCase, output) =>
  `| ${id} | ${measuredDate} | BASE-04 | ${stage} | \`${result.commit.slice(
    0,
    8,
...
const sessionsBlock = `<!-- BASE-04:SESSIONS:START -->
...
const experimentBlock = `<!-- BASE-04:EXPERIMENT:START -->
...
tracker = tracker.replace(
  /^\| BASE-04\s+\|.*$/m,
  `| BASE-04 | NumberInput·OptionalNumberInput | P0/P1 | DOM P95 ms | ${formatMs(
    visualBefore,
...
const partialRows = [
  [
    ...
    'P0/P1',
    'CTP ms',
    `\`${implementationShort}\`, 공통 숫자 입력 62곳 적용`,
  ],
  [
    'PICK-06',
    '카운터 애니메이션 생성·편집',
  ...
  [
    ...
    'P0/P1',
    'F95 ms/frame',
    `\`${implementationShort}\`, 그림자 수치 필드만 적용`,
  ],
];
...
tracker = tracker.replace(
  /^\| PROP-12\s+\|.*$/m,
  `| PROP-12 | 플러그인 설정 number·text·select | P1 | CTP ms | — | — | — | 실험 | \`2b9b6cf4\` select·\`${implementationShort}\` number 적용 |`,
);
tracker = tracker.replace(
  /^\| MODAL-02\s+\|.*$/m,
  `| MODAL-02 | 키·노트·카운터 설정 전체 | P1/P2 | CTP ms | — | — | — | 실험 | \`ad22c019\` 탭·\`2b9b6cf4\` Dropdown·\`${implementationShort}\` 숫자 입력 적용 |`,
);
...
console.info(
  `BASE-04 visual DOM commit P95: ${formatMs(visualBefore)}ms → ${formatMs(
    visualAfter,
  )}ms (${formatPercent(visualImprovement)})`,
);
