...
const variants = [
  [
    'legacy',
    'legacy-baseline',
    'benchmarks/results/grid-09-marquee-baseline.json',
  ],
  ['frame', 'improved', 'benchmarks/results/grid-09-marquee-improved.json'],
];
for (const [strategy, variant, output] of variants) {
  const result = spawnSync(
    npm,
    ['run', 'benchmark:interaction:grid-marquee:raw'],
    {
      cwd: root,
      env: {
        ...process.env,
...
if (!b || !a) throw new Error('GRID-09 측정 case 누락');
...
if (gain === null || gain <= 0) {
  throw new Error(`GRID-09 event P95 미개선: ${ms(before)}ms → ${ms(after)}ms`);
}
...
const resultBlock = `<!-- GRID-09:RESULT:START -->
#### GRID-09 마퀴 선택 최신 자동 측정

| 조건 | 값 |
| --- | --- |
| 측정 경로 | 실제 useGridMarquee + 렌더 DOM ${b.itemCount}개, mousemove ${
  b.burstSize
}회 burst |
...
const session = (id, stage, result, data, output) =>
  `| ${id} | ${date} | GRID-09 | ${stage} | \`${result.commit.slice(
    0,
    8,
  ...
  )}ms | [JSON](../${output}) | marquee burst proxy |`;
const sessionsBlock = `<!-- GRID-09:SESSIONS:START -->
| 세션 ID | 날짜 | 항목 ID | 단계 | 빌드·커밋 | 환경 | 시나리오·데이터 크기 | 반복 | P50 | P95 | 최대 | 보조 지표 | 원시 자료 | 비고 |
| --- | --- | --- | --- | --- | --- | --- | ---: | ---: | ---: | ---: | --- | --- | --- |
${session('GRID-09-LEGACY', '기준선', baseline, b, variants[0][2])}
${session('GRID-09-FRAME', '개선', improved, a, variants[1][2])}
<!-- GRID-09:SESSIONS:END -->`;
const experimentBlock = `<!-- GRID-09:EXPERIMENT:START -->
...
tracker = tracker.replace(
  /^\| GRID-09\s+\|.*$/m,
  `| GRID-09 | 마퀴 선택 | P0/P1 | F95 ms/frame | ${ms(before)} | ${ms(
    after,
  )} | ${pct(gain)} | 검증 | \`${short}\`, 최신 좌표 frame coalescing |`,
);
...
console.info(
  `GRID-09 event P95: ${ms(before)}ms → ${ms(after)}ms (${pct(gain)})`,
);
