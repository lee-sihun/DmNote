...
fn preset_load_from_path(
    ...
    ticket.run(move || {
        ...
        let (transaction, _) = state.commit_editor_transaction_preserving_runtime_counters(
            app,
            |current_key_counters| {
                state.store.commit_preset_editor_transaction_with_admission(
