...
/// tabOrder가 없던 시절 데이터의 표시 순서 복원
///
/// 당시에는 단일 표시 순서가 없었고 설정 목록은 custom_tabs를 역순으로,
/// 오버레이 우클릭 메뉴는 저장 순서 그대로 표시
/// 탭 배치를 관리하던 설정 목록에서 새 탭을 위에 보여주던 사용자 경험을 기준으로 선택
///
/// 저장 데이터와 프리셋 모두 같은 시절 형식이므로 같은 함수 사용
/// 경로별 복원 결과가 갈리면 같은 프리셋도 진입점에 따라 다르게 표시
