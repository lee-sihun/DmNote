...
struct OverlayResizeExecutionState {
    by_window_label: HashMap<String, OverlayResizeWindowGeneration>,
}
...
impl OverlayResizeWindowGeneration {
    fn new(active_session: u64) -> Self {
        Self {
            active_session,
            high_gen: None,
            stale_session: None,
...
impl AppState {
    ...
    pub fn obs_hide_overlay(&self, app: &AppHandle) {
        ...
        *self.obs_previous_overlay_visible.write() = Some(was_visible);
        if let Err(error) = self.overlay_hit.set_visible(app, false) {
            log::warn!("[ObsBridge] 히트 창 숨김 실패: {error:#}");
        }
    ...
    pub fn set_overlay_visibility(&self, app: &AppHandle, visible: bool) -> Result<()> {
        ...
        let previous_visible = *self.overlay_visible.read();
        ...
        if let Err(persist_err) = self.store.update(|state| {
            ...
            } else if let Err(error) = self.overlay_hit.set_visible(app, previous_visible) {
    ...
    fn resize_overlay_validated(
        ...
        resize_execution.mark_executed(caller_window_label, request_session, request_gen);
        ...
        window.on_window_event(move |event| match event {
            WindowEvent::CloseRequested { api, .. } => {
                if force_close_flag.load(Ordering::SeqCst) {
                    ...
                    *overlay_visible.write() = false;
                    let _ = overlay_hit.set_visible(&app_handle, false);
                } else {
                    api.prevent_close();
                    if let Err(err) =
                    ...
                    if let Err(err) = store.update(|state| {
                        ...
                        } else if let Err(error) = overlay_hit.set_visible(&app_handle, true) {
            ...
            WindowEvent::Focused(false) => {
                ...
                if let Err(err) = overlay_hit.reconcile(&app_handle) {
                    log::warn!("failed to reconcile overlay hit windows: {err:#}");
                }
            }
...
mod tests {
    ...
    fn different_window_labels_keep_independent_active_sessions() {
        ...
        assert_eq!(
            (overlay_state.active_session, overlay_state.high_gen),
            (200, Some(1))
        );
        assert_eq!(
            (obs_state.active_session, obs_state.high_gen),
            (100, Some(2))
        );
    }

    #[test]
    fn missing_session_or_generation_bypasses_the_window_gate() {
        let mut execution = OverlayResizeExecutionState::default();
        let window_label = "overlay";
