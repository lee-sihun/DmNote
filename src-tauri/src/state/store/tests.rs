use super::{
    ...
    plugin_instances_storage_key, project_history_key_counters, purge_expired_trash_sessions_at,
    recover_interrupted_processed_wav_replacements_with, recover_interrupted_sound_deletions_with,
    stage_sound_files_for_deletion, sweep_unreferenced_asset_files, system_time_millis, AppStore,
...
use crate::{
    ...
    models::{
        ...
        CustomCss, CustomFont, CustomTab, EditorBoundsV1, EditorCommitOrigin, EditorCommitRequest,
        ...
        EditorZUpdateV1, FontSettings, FontType, GestureCommitRequest,
...
fn counter_fill_solid(color: &str) -> crate::models::EditorCounterFillIntentV1 {
    crate::models::EditorCounterFillIntentV1::Solid(crate::models::EditorCounterFillSolidIntentV1 {
        color: color.to_string(),
    })
}
...
fn legacy_position_replacements_reject_noncanonical_ids_atomically() {
    ...
    fn with_raw_position_id(mut payload: serde_json::Value, id: Option<&str>) -> serde_json::Value {
    ...
    let invalid_cases = vec![
        (
            ...
            PositionReplacement::Stat(
                serde_json::from_value(with_raw_position_id(stat_payload.clone(), None)).unwrap(),
            ),
        ),
        (
            EditorField::GraphPositions,
    ...
    crate::state::native_element_id::validate_document_element_ids(&store.editor_get().document)
        .unwrap();
    store.flush_and_shutdown().unwrap();
    let _ = std::fs::remove_dir_all(dir);
}
...
fn v2_editor_commit_rejects_invalid_ids_atomically_and_keeps_read_event_v1() {
    ...
    merged_duplicate.get_mut("4key").unwrap()[0].id = baseline.document.stat_positions["4key"][0]
        .position
        .id
        .clone();
    ...
    let no_change = store
        .commit_editor_document(editor_ops_request(
            ...
            vec![
                ...
                patch_layer_name_op(EditorElementTypeV1::Key, layer_name_id, Some("Named layer")),
            ],
        ))
        .unwrap();
    assert!(no_change.result.changed_fields.is_empty());
...
fn common_option_batches_replay_and_round_trip_all_native_types() {
    ...
    let original_font_strikethrough = redone.knob_positions["4key"][0].position.font_strikethrough;
    let font_ops = vec![
        patch_property_op(
            EditorElementTypeV1::Key,
...
fn note_paint_batch_replays_and_round_trips_one_history_entry() {
    ...
    .unwrap();
    let legacy_note_wire = serde_json::to_vec(&setup.document.key_positions["4key"][0]).unwrap();
    let ops = vec![
        patch_property_op(
            EditorElementTypeV1::Key,
            &ids[0],
    ...
    reused.ops = Some(vec![patch_property_op(
        EditorElementTypeV1::Key,
        &ids[0],
        EditorElementPropertyPatchV1::NotePaint(crate::models::EditorNotePaintIntentV1::Opacity(
    ...
    store.flush_and_shutdown().unwrap();
    drop(store);
    let reloaded = crate::state::migration::load_store_from_path(&dir.join("store.json")).unwrap();
...
fn legacy_delete_patch_keeps_key_pair_and_empty_group_cleanup_atomic() {
    ...
    grouped.key_positions.get_mut("4key").unwrap()[0].group_id = Some("delete-group".to_string());
...
fn delete_ops_remove_all_types_in_request_order_and_round_trip_one_history_entry() {
    ...
    setup.key_positions.get_mut("4key").unwrap()[1].group_id = Some("survive-group".to_string());
    setup.key_positions.get_mut("4key").unwrap()[2].group_id = Some("survive-group".to_string());
...
fn delete_op_keeps_a_group_definition_while_another_native_member_survives() {
    ...
    let store = AppStore::initialize_in_dir(&dir).unwrap();
    let mut document = store.editor_get().document;
    document.key_positions.get_mut("4key").unwrap()[0].group_id = Some("shared-group".to_string());
    document.key_positions.get_mut("4key").unwrap()[1].group_id = Some("shared-group".to_string());
...
fn editor_group_bounds_reject_every_invalid_batch_atomically() {
    ...
    for (ops, expected_code) in cases {
        let error = store
            .commit_editor_document(editor_ops_request(0, uuid::Uuid::new_v4().to_string(), ops))
            .unwrap_err();
        assert_eq!(
            error.details.unwrap().validation_code.as_deref(),
            Some(expected_code)
...
fn css_watcher_reload_after_undo_preserves_latest_canonical_and_invalidates_redo() {
    ...
    let current_counters = store.snapshot().key_counters;
    let redo =
        store.apply_history_operation(HistoryDirection::Redo, &redo_id, &current_counters, || {});
    drop(barrier);
...
#[test]
fn tab_history_transaction_normalizes_metadata_before_recording() {
    for preset in [false, true] {
        for (bar_count, expected_bar_count) in [(0, 1), (u8::MAX, 4)] {
            let directory = tempfile::tempdir().unwrap();
            let store = AppStore::initialize_for_test(directory.path()).unwrap();
            let initial = store.snapshot();
            let expected_order = ["6key", "4key", "5key", "8key"].map(str::to_string);
            let updater = |data: &mut AppStoreData| {
                data.tab_order = ["6key", "6key", "missing"].map(str::to_string).to_vec();
                data.bar_count = bar_count;
                Ok(())
            };
            let origin = EditorCommitOrigin::LegacyAdapter("tab_metadata_test".to_string());
            let transaction = if preset {
                store.commit_preset_editor_transaction(
                    origin,
                    &[],
                    initial.key_counters.clone(),
                    updater,
                )
            } else {
                store.commit_aux_editor_transaction(
            ...
            .unwrap();
            drop(transaction);
            assert_eq!(store.snapshot().tab_order, expected_order);
            assert_eq!(store.snapshot().bar_count, expected_bar_count);
            ...
            for direction in [HistoryDirection::Undo, HistoryDirection::Redo] {
                ...
                match direction {
                    HistoryDirection::Undo => {
                        assert_eq!(snapshot.tab_order, initial.tab_order);
                        assert_eq!(snapshot.bar_count, initial.bar_count);
                    }
                    HistoryDirection::Redo => {
                        assert_eq!(snapshot.tab_order, expected_order);
                        assert_eq!(snapshot.bar_count, expected_bar_count);
                    }
                }
                ...
                .unwrap();
                assert_eq!(persisted.tab_order, snapshot.tab_order);
                assert_eq!(persisted.bar_count, snapshot.bar_count);
            }
            store.flush_and_shutdown().unwrap();
        }
    }
...
#[test]
fn tab_history_restore_clamps_out_of_range_bar_count() {
    for preset in [false, true] {
        for (bar_count, expected_bar_count) in [(0, 1), (u8::MAX, 4)] {
            let directory = tempfile::tempdir().unwrap();
            let store = AppStore::initialize_for_test(directory.path()).unwrap();
            // 과거 스냅샷의 범위 밖 표시 개수 복원
            store.update(|data| data.bar_count = bar_count).unwrap();
            let updater = |data: &mut AppStoreData| {
                data.bar_count = 2;
                Ok(())
            };
            let origin = EditorCommitOrigin::LegacyAdapter("tab_restore_test".to_string());
            let transaction = if preset {
                store.commit_preset_editor_transaction(
                    origin,
                    &[],
            ...
            for (direction, expected) in [
                (HistoryDirection::Undo, expected_bar_count),
                (HistoryDirection::Redo, 2),
            ] {
                let operation_id = uuid::Uuid::new_v4().to_string();
                let barrier = gate.close(&operation_id).unwrap();
                store
                    ...
                    .unwrap();
                drop(barrier);
                assert_eq!(store.snapshot().bar_count, expected);
            }
            store.flush_and_shutdown().unwrap();
        }
    }
    ...
    let delete = store
        .commit_aux_editor_reset_transaction_with_admission(
            AuxEditorResetTransactionOptions {
                ...
                origin: EditorCommitOrigin::LegacyAdapter("custom_tab_plugin_delete".to_string()),
                touched_fields: &[
                    EditorField::Keys,
                    EditorField::KeyPositions,
                    EditorField::StatPositions,
...
fn css_watcher_reload_revalidates_after_waiting_for_store_lock() {
    ...
    let mutation = thread::spawn(move || commit_css_reload(&mutation_store, watched_path, &loaded));
...
fn stale_key_publisher_cannot_overwrite_newer_counter_runtime() {
    ...
    let stale_emitter = TestCounterEmitter::new(Arc::clone(&events), mode.clone(), new_key.clone());
    state
        .apply_committed_editor_key_runtime(
            &stale_emitter,
...
fn writer_failure_keeps_snapshot_and_disk_unchanged_and_does_not_leak() {
    ...
    let after_disk: AppStoreData = serde_json::from_slice(&std::fs::read(&path).unwrap()).unwrap();
...
fn reset_snapshot_emit_precedes_waiting_increment_emit() {
    ...
    let reset_handle =
        std::thread::spawn(move || reset_state.reset_key_counters(reset_emitter.as_ref(), None));
    snapshot_emitted_rx
...
fn queued_increment_survives_same_generation_noop_key_publication() {
    ...
    let state = Arc::new(AppState::initialize(store).unwrap());
    let events = Arc::new(Mutex::new(Vec::new()));
    let mutation_emitter = TestCounterEmitter::new(Arc::clone(&events), mode.clone(), key.clone());
    ...
    let history_emitter = TestCounterEmitter::new(Arc::clone(&events), mode.clone(), key.clone());
    ...
    let noop = state
        .store
        .commit_legacy_editor_transaction(
            EditorCommitOrigin::LegacyAdapter("same_generation_noop_key_publication".to_string()),
            &[EditorField::Keys],
            |_| Ok(()),
        )
        .unwrap();
...
fn trash_purge_removes_only_sessions_older_than_thirty_days() {
    ...
    let retained_time = now
        ...
        .unwrap();
    let expired_dir = trash_dir.join(format!("{}-1", system_time_millis(expired_time).unwrap()));
...
fn unknown_pending_wav_presence_keeps_journal() {
    ...
    let result = store.recover_pending_processed_wav_replacement_with(&sounds_dir, |_path| {
        match checks.get() {
            0 => {
                checks.set(1);
                Ok(false)
...
fn orphan_sweep_continues_recovery_and_protects_failed_backup() {
    ...
    let recovery = recover_interrupted_processed_wav_replacements_with(&sounds_dir, |from, to| {
