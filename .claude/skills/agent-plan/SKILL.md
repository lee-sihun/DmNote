...
- **에스컬레이션**: 아키텍처 전면 개편, 데이터 모델/영속성 설계 등 실패 비용이 큰 계획 검증은 `model` 지정을 **생략**하여 메인 모델(상위 티어)을 상속시킵니다.
...
- 사실 수집(파일 목록·콜체인 정찰)만 필요한 보조 탐색은 `Explore` 에이전트 + `model: "sonnet"`으로 저비용 병렬 처리할 수 있습니다.
